...
class TestQuestionsStore:
    ...
    async def test_create_question(
        self, cli, store: Store, theme_1: ThemeModel, answers: list[AnswerModel]
    ):
    ...
    async def test_create_question_no_theme(
        self, cli, store: Store, answers: list[AnswerModel]
    ):
        question_title = "title"
    ...
    async def test_create_question_unique_title_constraint(
        self, cli, store: Store, question_1: QuestionModel, answers: list[AnswerModel]
    ):
        with pytest.raises(IntegrityError) as exc_info:
    ...
    async def test_get_question_by_title(self, cli, store: Store, question_1: QuestionModel):
        assert question_1 == await store.quizzes.get_question_by_title(question_1.title)

    async def test_list_questions(
        self, cli, store: Store, question_1: QuestionModel, question_2: QuestionModel
    ):
...
class TestQuestionAddView:
    async def test_success(self, authed_cli, theme_1: ThemeModel):
            ...
            data=question2dict(
                QuestionModel(
                    ...
                    answers=[
                        ...
                        AnswerModel(title="8", is_correct=True),
                    ],
                )
            )
        )
...
class TestQuestionListView:
    ...
    async def test_several_questions(
        self, authed_cli, question_1: QuestionModel, question_2
    ):
        resp = await authed_cli.get("/quiz.list_questions")
        assert resp.status == 200
        data = await resp.json()
