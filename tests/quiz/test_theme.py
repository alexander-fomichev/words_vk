...
class TestThemeStore:
    ...
    async def test_create_theme(self, cli, store: Store):
        ...
        assert type(theme) is ThemeModel
...
class TestThemeAddView:
    ...
    async def test_success(self, store: Store, authed_cli):
        ...
        assert data == ok_response(
            data=theme2dict(ThemeModel(id=data["data"]["id"], title="web-development")),
        )
    ...
    async def test_conflict(self, authed_cli, theme_1: ThemeModel):
        resp = await authed_cli.post(
            "/quiz.add_theme",
            json={
                "title": theme_1.title,
...
class TestIntegration:
    async def test_success(self, authed_cli):
        ...
        data = await resp.json()
        assert data == ok_response(
            data={"themes": [theme2dict(ThemeModel(id=theme_id, title="integration"))]}
        )
