...
from app.store.vk_api.dataclasses import UpdateObject
...
class BotManager(BaseAccessor):
    ...
    async def handle_updates(self, update: UpdateObject):
        peer_id = update.peer_id
        cmd = update.body.lower()
        ...
        if cmd == "!статус":
            ...
            if game.game_db.status == "init":
                ...
                if cmd == "я":
                    await game.add_player(user_id=update.user_id)
                else:
                    await registration_message(self.app.store.vk_api, peer_id, game.game_db.setting.timeout,
                                               game.game_db.setting.title)
            elif game.game_db.status == "started":
                if update.user_id == game.game_db.current_move:
                    await game.check_word(cmd)
                else:
                    name = game.get_player_name_by_user_id(game.game_db.current_move)
                    last_word = game.game_db.last_word
                    await player_move_message(self.app.store.vk_api, game.peer_id, name, last_word,
                                              game.game_db.setting.timeout)
            elif game.game_db.status == "vote_word":
                name = game.get_player_name_by_user_id(update.user_id)
                if update.user_id == game.game_db.current_move:
                    await vote_self_message(self.app.store.vk_api, game.peer_id, name)
                else:
                    if cmd == "да":
                        await game.add_vote(True, update.user_id)
                    elif cmd == "нет":
                        await game.add_vote(False, update.user_id)
                    else:
                        await vote_message(self.app.store.vk_api, peer_id, game.game_db.vote_word,
                                           game.game_db.setting.timeout)
        return
